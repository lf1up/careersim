...
export const analyticsOverviewSchema = z.object({
  ...
  reports: z.object({
    ...
    skill_averages: z.array(skillAverageSchema).max(20),
    trend: z.array(scoreTrendPointSchema).max(20),
    top_strengths: z.array(phraseCountSchema).max(5),
    top_improvement_areas: z.array(phraseCountSchema).max(5),
    tones: z.array(toneCountSchema).max(10),
  }),
  per_simulation: z.array(simulationBreakdownSchema).max(50),
});
