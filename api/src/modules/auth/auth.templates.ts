...
<html>
  <body style="font-family: -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, sans-serif; background: #f5f1e8; padding: 24px; color: #1a1a1a;">
    <div style="max-width: 480px; margin: 0 auto; background: #fff; border: 2px solid #000; box-shadow: 4px 4px 0 #000; padding: 24px;">
      ...
      <p style="margin: 24px 0 0; font-size: 12px; color: #666;">— ${PRODUCT}</p>
    </div>
  </body>
</html>`;
}
...
export function verifyEmailMail(email: string, code: string): MailMessage {
  const subject = `Your ${PRODUCT} confirmation code: ${code}`;
  const text = `Welcome to ${PRODUCT}!
...
It expires in 10 minutes. If you didn't start a signup, you can safely ignore this email.`;
  const html = shell(
    'Confirm your email',
    `<p>Welcome to ${PRODUCT}! Enter this 6-digit code to finish creating your account:</p>
...
export function loginLinkMail(email: string, url: string): MailMessage {
  const subject = `Your ${PRODUCT} sign-in link`;
  const text = `Click the link below to sign in to ${PRODUCT}:
...
export function resetPasswordMail(email: string, url: string): MailMessage {
  const subject = `Reset your ${PRODUCT} password`;
  const text = `A password reset was requested for your ${PRODUCT} account.
...
It expires in 30 minutes. If you didn't request this, your account is still safe — just ignore this email.`;
  const html = shell(
    'Reset your password',
    `<p>A password reset was requested for your ${PRODUCT} account. Click the button below to choose a new password. The link expires in 30 minutes.</p>
...
export function changeEmailMail(newEmail: string, code: string): MailMessage {
  const subject = `Confirm your new ${PRODUCT} email: ${code}`;
  const text = `We received a request to change a ${PRODUCT} account's email to this address.
...
It expires in 10 minutes. If you didn't request this, you can safely ignore this email.`;
  const html = shell(
    ...
    `<p>We received a request to change a ${PRODUCT} account's email to this address. Enter this code back in the app to finish the change:</p>
     <p style="font-size: 28px; letter-spacing: 6px; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; margin: 16px 0; padding: 12px; border: 2px solid #000; background: #fffbe6; text-align: center;">${escapeHtml(
       code,
     )}</p>
