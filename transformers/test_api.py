...
AUTH_TOKEN = os.getenv('AUTH_TOKEN', 'default-dev-token-change-in-production-min-32-chars')
...
TEST_DATA = {
    ...
    "sequence": [
        ...
        },
        {
            ...
            "candidate_labels": ["technical support", "billing question", "general inquiry"]
        },
        {
            ...
            if cache_info:
                ...
                if cache_info.get('cache_exists'):
                    if 'cache_size_mb' in cache_info:
                        print(f"   📦 Cache size: {cache_info['cache_size_mb']} MB ({cache_info.get('cached_files', 0)} files)")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for i, test_case in enumerate(test_cases, 1):
            try:
                ...
                response = await client.post(f"{BASE_URL}/{endpoint}", json=test_case, headers=get_headers())
                end_time = time.time()
                ...
                if "top_prediction" in data:
                    ...
                    if len(data["predictions"]) > 1:
                        ...
                        for j, pred in enumerate(data["predictions"][:3], 1):
                            print(f"         {j}. {pred['label']}: {pred['confidence']:.4f}")
                else:
                    # Simple result format
                    print(f"      Result: {data['label']} (confidence: {data['confidence']:.4f})")
...
async def main():
    ...
    if AUTH_REQUIRED:
        print(f"🔑 Using token: {AUTH_TOKEN[:8]}...{AUTH_TOKEN[-8:] if len(AUTH_TOKEN) > 16 else AUTH_TOKEN}")
    ...
    if passed_tests == total_tests:
        ...
        print("   • Check if the service is running on port 8000")
