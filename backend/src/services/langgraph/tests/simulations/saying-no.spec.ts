/**
 * End-to-End Test: Saying "No" to Extra Work with Sarah Jenkins
 * 
 * Tests the "Saying 'No' to Extra Work" simulation with up to 50 conversation turns.
 * Evaluates boundary setting, empathetic communication, and assertiveness.
...
describe('Saying "No" to Extra Work Simulation (Sarah Jenkins)', () => {
  // Global setup - start server once
  beforeAll(async () => {
    console.log('\n' + '═'.repeat(80));
    console.log('🚀 SAYING "NO" TO EXTRA WORK TEST SUITE - SETUP');
  ...
  afterAll(async () => {
    console.log('\n' + '═'.repeat(80));
    console.log('🧹 SAYING "NO" TEST SUITE - CLEANUP');
    console.log('═'.repeat(80));
  ...
  test('should complete boundary setting conversation and achieve all goals', async () => {
    console.log('\n\n');
    console.log('╔' + '═'.repeat(78) + '╗');
    console.log('║' + ' Saying "No" to Extra Work - All Goals'.padEnd(78) + '║');
    console.log('╚' + '═'.repeat(78) + '╝');
    
    // Load simulation details
    const simulation = await loadSimulationBySlug('saying-no-sarah');
    ...
    console.log('\n✅ Saying "No" simulation test completed successfully');
  }, TEST_TIMEOUT);
});
