/**
 * End-to-End Test: Delegating a Task to Chloe Davis
 * 
 * Tests the "Delegating a Task" simulation with up to 50 conversation turns.
 * Evaluates delegation skills, clear communication, and confidence building.
...
describe('Delegating a Task Simulation (Chloe Davis)', () => {
  // Global setup - start server once
  beforeAll(async () => {
    console.log('\n' + '═'.repeat(80));
    console.log('🚀 DELEGATING A TASK TEST SUITE - SETUP');
  ...
  afterAll(async () => {
    console.log('\n' + '═'.repeat(80));
    console.log('🧹 DELEGATING TASK TEST SUITE - CLEANUP');
    console.log('═'.repeat(80));
  ...
  test('should complete delegation conversation and achieve all goals', async () => {
    console.log('\n\n');
    console.log('╔' + '═'.repeat(78) + '╗');
    console.log('║' + ' Delegating a Task - All Goals'.padEnd(78) + '║');
    console.log('╚' + '═'.repeat(78) + '╝');
    
    // Load simulation details
    const simulation = await loadSimulationBySlug('delegating-task-chloe');
    ...
    console.log('\n✅ Delegating Task simulation test completed successfully');
  }, TEST_TIMEOUT);
});
