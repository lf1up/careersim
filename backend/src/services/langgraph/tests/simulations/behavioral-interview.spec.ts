/**
 * End-to-End Test: Behavioral Interview with Brenda Vance
 * 
 * Tests the "The Behavioral Interview" simulation with up to 50 conversation turns.
 * Evaluates against success criteria and conversation goals.
...
const MAX_TURNS = 50; // Increased from 18 to 50

describe('Behavioral Interview Simulation (Brenda Vance)', () => {
  // Global setup - start server once
  beforeAll(async () => {
    console.log('\n' + '═'.repeat(80));
    console.log('🚀 BEHAVIORAL INTERVIEW TEST SUITE - SETUP');
  ...
  afterAll(async () => {
    console.log('\n' + '═'.repeat(80));
    console.log('🧹 BEHAVIORAL INTERVIEW TEST SUITE - CLEANUP');
    console.log('═'.repeat(80));
  ...
  test('should complete behavioral interview and achieve all goals', async () => {
    console.log('\n\n');
    console.log('╔' + '═'.repeat(78) + '╗');
    console.log('║' + ' TEST 1: Complete Behavioral Interview - All Goals'.padEnd(78) + '║');
    console.log('╚' + '═'.repeat(78) + '╝');
    
    // Load simulation details
    const simulation = await loadSimulationBySlug('behavioral-interview-brenda');
    ...
    const evaluation = generateEvaluationReport(
      simulation.slug,
      Math.floor(result.turns.length / 2), // Count exchanges, not individual messages
      successCriteriaEval,
      goalEvaluations,
      overallScore
    );
    ...
    // Assertions
    expect(result.turns.length).toBeGreaterThanOrEqual(10); // At least 5 exchanges
    expect(evaluation.overallScore).toBeGreaterThanOrEqual(50); // Lower threshold for realistic tests
    
    // Check that goals were progressed
    ...
    // If the conversation stopped early, it means all goals were achieved
    ...
    console.log('\n✅ Behavioral Interview simulation test completed successfully');
  }, TEST_TIMEOUT);
});
