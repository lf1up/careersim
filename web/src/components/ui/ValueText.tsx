...
function detectValueKind(value: string | number): ValueKind {
  ...
  if (
    stringValue.endsWith('ms') ||
    stringValue.endsWith('s') ||
    stringValue.endsWith('m') ||
    stringValue.endsWith('h')
  )
    return 'time';
...
export const ValueText: React.FC<{ value: string | number; className?: string }> = ({
  value,
  className,
}) => {
  ...
  return (
    <span className={`font-monoRetro ${colorClass}${className ? ` ${className}` : ''}`}>
      {value}
    </span>
  );
};
