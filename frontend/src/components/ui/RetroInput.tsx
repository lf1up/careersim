...
export const RetroInput: React.FC<RetroInputProps> = ({ label, hint, error, containerClassName, className, id, ...props }) => {
  ...
  return (
    <div className={clsx('w-full', containerClassName)}>
      {label && (
        <label htmlFor={inputId} className="block text-sm font-semibold mb-1 text-retro-ink dark:text-retro-ink-dark">
          {label}
        </label>
      )}
      <input id={inputId} aria-invalid={!!error} aria-describedby={describedById} className={clsx('retro-input w-full', className)} {...props} />
      {hint && !error && <p id={`${inputId}-hint`} className="text-xs mt-1 font-monoRetro text-secondary-600 dark:text-secondary-400">{hint}</p>}
      {error && <p id={`${inputId}-error`} className="text-xs mt-1 text-red-600 dark:text-red-400">{error}</p>}
    </div>
  );
};
...
export const RetroSelect: React.FC<RetroSelectProps> = ({ label, hint, error, containerClassName, className, id, children, ...props }) => {
  ...
  return (
    <div className={clsx('w-full', containerClassName)}>
      {label && (
        <label htmlFor={selectId} className="block text-sm font-semibold mb-1 text-retro-ink dark:text-retro-ink-dark">
          {label}
        </label>
      )}
      ...
      {hint && !error && <p className="text-xs mt-1 font-monoRetro text-secondary-600 dark:text-secondary-400">{hint}</p>}
...
export const RetroTextArea: React.FC<RetroTextAreaProps> = ({ label, hint, error, containerClassName, className, id, ...props }) => {
  ...
  return (
    <div className={clsx('w-full', containerClassName)}>
      {label && (
        <label htmlFor={textAreaId} className="block text-sm font-semibold mb-1 text-retro-ink dark:text-retro-ink-dark">
          {label}
        </label>
      )}
...
export const RetroCheckbox: React.FC<RetroCheckboxProps> = ({ label, containerClassName, className, ...props }) => {
  return (
    <label className={clsx('inline-flex items-center gap-2 select-none', containerClassName)}>
      <input type="checkbox" className={clsx('appearance-none w-4 h-4 border-2 border-black dark:border-retro-ink-dark shadow-retro-2 dark:shadow-retro-dark-2 checked:bg-black dark:checked:bg-retro-ink-dark', className)} {...props} />
      {label && <span className="text-sm text-retro-ink dark:text-retro-ink-dark">{label}</span>}
    </label>
  );
};
...
export const RetroToggle: React.FC<RetroToggleProps> = ({ label, containerClassName, className, ...props }) => {
  return (
    <label className={clsx('inline-flex items-center gap-3 select-none', containerClassName, className)}>
      <span className="relative inline-block w-10 h-6 border-2 border-black dark:border-retro-ink-dark shadow-retro-2 dark:shadow-retro-dark-2">
        <input type="checkbox" className="sr-only peer" {...props} />
        <span className="absolute top-0 left-0 h-full w-1/2 bg-black dark:bg-retro-ink-dark transition-all peer-checked:translate-x-full" />
      </span>
      {label && <span className="text-sm text-retro-ink dark:text-retro-ink-dark">{label}</span>}
    </label>
  );
