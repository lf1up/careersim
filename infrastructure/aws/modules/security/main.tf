...
resource "aws_security_group" "services" {
  ...
  # Allow inbound from ALB on backend port 8000
  ingress {
    from_port       = 8000
    to_port         = 8000
    protocol        = "tcp"
  ...
  # Allow inter-service traffic (backend -> rag/transformers on 8001/8002)
  ingress {
    from_port       = 8001
    to_port         = 8002
    protocol        = "tcp"
  ...
  tags = merge(var.tags, { Name = "${local.name_prefix}-services-sg" })
}
